    ...
    public class ExcelImportEstab
    {
        ...
        public int importToSQL(string strExcelConn)
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["SQLServer2005DBConnectionString"].ToString()))
            {
                ...
                " SELECT @idDepartamento = [id] FROM DEPARTAMENTOS WHERE departamento = '" + dtExcel.Rows[i]["departamento"] + "' " +
                " SELECT @idLocalidad = [id] FROM LOCALIDADES WHERE localidad = '" + dtExcel.Rows[i]["localidad"] + "' " +
                    " INSERT INTO ESTABLECIMIENTOS"+ 
                    " (codigo,nombre,telefono,telefono2,departamento,localidad,director,domicilio,barrio,cp,email,web,plannacer) "+
